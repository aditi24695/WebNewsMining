...
from multiprocessing import Process, Lock, Manager,freeze_support, freeze_support, Value, Array
        ...
        #        print("Category of children")
         #       print(children.category)
                if A['category'] == children.category:
                    ...
                    print(" Returned after protype updtaion")
                   # if (coll_content.isReceivedData()):
        ...
        for children in root.children:
            if children.category == A['category']:
                for prototype in children.children:
                    sum = sum + (prototype.prototype['tfidf'][i])**2
                return sum
            ...
            try:
                if prototype.prototype == '':
                    for leaf_p in prototype.children:
                        up_potential = ((self.calcQ(A) - 1)*leaf_p.prototype['potential'])/((self.calcQ(A)-1)+((self.calcQ(A)-2)*(1/(leaf_p.prototype['potential'])-1))+(self.calcCosDist(A['tfidf'],leaf_p.prototype['tfidf'])))
    ...
    def combinePrototype(self, children):
        ...
        for prototype in children.children:
            if prototype.prototype is '':
                for children in prototype:
                    prototype.parent.parent.set_children()
                    prototype.removeChildren(children)
                class_prototype.append(prototype)
...
class classifyPrototype():
    def classify(self):
        ...
        buffer = []
        A = {}
        tokens = preprocess.tokenize(description)
        ...
        for children in root:
            if children.children.prototype  == '':
                if (children.children.tokens , tfidf) > 0.95:
                ...
                cosDist = buildTree.calcCosDist(tfidf, children.children.prototype['tfidf'])
                if cosDist < minVal:
                    category = children.children.prototype['tfidf']
                    minVal = cosDist
        print("category")
        return category
 ...
 #           lock.acquire()
  ...
  #          lock.release()
    ...
    def collectData(self):
        ...
        global  data_buff
        data_ind = {}
        with atomic_write("sample.csv", overwrite=True)as f:
            ...
            for children in root.children:
                ...
                for prototype in children.children:
                    if not len(prototype.buffer) == 0:
                        return True
        return False
# Exception in thread Thread-363316:
# Traceback (most recent call last):
#   File "/usr/lib/python3.5/threading.py", line 914, in _bootstrap_inner
#     self.run()
#   File "/usr/lib/python3.5/threading.py", line 862, in run
#     self._target(*self._args, **self._kwargs)
#   File "/home/chikki/PycharmProjects/admassignment1/anytime.py", line 656, in isDataReceived
#     if len(temp_data_buff) == 0 and self.has_buffer_filled():
#   File "/home/chikki/PycharmProjects/admassignment1/anytime.py", line 722, in has_buffer_filled
#     print(child.buffer)
